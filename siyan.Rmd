...
```{r}
ridge.model = glmnet(x, y, alpha = 0, lambda = exp(seq(-1, 10, length = 100)))
...
cv.ridge = cv.glmnet(x, y,
                     ...
                     type.measure = "mse")
plot(cv.ridge)
best.lambda = cv.ridge$lambda.min # 3.394723
predict(ridge.model,s = best.lambda, type = "coefficients")
```
