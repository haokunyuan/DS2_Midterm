...
df_useful$weekly_sales = log(df_useful$weekly_sales- min(df_useful$weekly_sales)+ 0.0001) 
df_useful$weekly_sales %>% hist()
summary(df_useful$weekly_sales)

foo = quantile(df_useful$weekly_sales, seq(0,1,0.1)) 
...
cv.lasso <- cv.glmnet(x, y,
                      alpha = 1,
                      lambda = exp(seq(-50, -3, length=100)),
                      type.measure = "mse",
                      parallel = TRUE)
plot(cv.lasso)
cv.lasso$lambda.1se
