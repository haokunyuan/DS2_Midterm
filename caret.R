...
selectNcomp(pcr.mod, method = c("onesigma"), #randomization
            ncomp = pcr.mod$ncomp)
...
selectNcomp(pls.mod, method = c("onesigma"), #randomization
            ncomp = pls.mod$ncomp)
...
#pcr
pred_tr_pcr = predict(pcr.mod, newdata = test.data, ncomp = 131)
mean((pred_tr_pcr - newy)^2) %>% sqrt()
#pls
pred_tr_pls = predict(pls.mod, ewdata = test.data, ncomp = 47)
